...
if options.naf:
    ...
    for line in listtxt: 
        ...
        if not options.bulk : 
            ...
            temp = temp.replace('@EXESH',str(os.getcwd())+"/"+wrapsub).replace('@LOGS',str(logdir)).replace('@time','60*60*9')
    ...
    if options.bulk : 
        ...
        temp = temp.replace('@EXESH',str(os.getcwd())+'/$(Chunk)/wrapnanoPost.sh').replace('@LOGS',str(logdir)).replace('@time','60*60*9').replace('Queue 1','queue Chunk matching dirs '+outdir+'/*')
