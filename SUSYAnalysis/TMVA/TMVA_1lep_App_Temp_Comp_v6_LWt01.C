...
//reader->AddVariable("nDeepTop_medium" ,&nDeepTop_medium );
   ...
   for (std::map<std::string,int>::iterator it = Use.begin(); it != Use.end(); it++) {
      if (it->second) {
         ...
         TString weightfile = "/nfs/dust/cms/user/turkot/SUSY/CMSSW_9_4_6_patch1/src/CMGTools/SUSYAnalysis/TMVA/@MASSPOINT/weights/TMVAClassification_BDT.weights.xml";
